...
function isProtoLang(code: string): boolean {
  return code.endsWith("-pro") || ["ine-pro","gem-pro","gmw-pro","sit-pro","azc-nah-pro","poz-pol-pro"].includes(code);
}

function showTip(ev: MouseEvent, d: { word: string; lang: string; etym_text?: string }): void {
  const tip = document.getElementById("tip");
  if (!tip) return;
  const meta = getLang(d.lang);
  document.getElementById("tt-w")!.textContent = d.word;
  document.getElementById("tt-l")!.textContent = meta.name + (meta.family ? " · " + meta.family : "");
  document.getElementById("tt-b")!.textContent = (d.etym_text ?? "").slice(0, 130);
...
/**
 * Builds a quadratic bezier screen-space path for a map arc.
 * The control point is offset perpendicular to the chord,
 * direction and magnitude driven by the RouteStyle.
 *
 * TODO: Replace with historical waypoint routing per category.
 * Currently logs to console via getRouteStyle() for scaffold verification.
 */
function buildArcPath(
  ...
  // Perpendicular offset — sign encodes bend direction
...
/** Shorten arc endpoint so arrow doesn't overlap node circle */
function arcEndShortened(sx: number, sy: number, mx: number, my: number, tx: number, ty: number, r: number): [number, number] {
  const tdx = tx - mx, tdy = ty - my;
  const tlen = Math.sqrt(tdx * tdx + tdy * tdy) || 1;
  return [tx - (tdx / tlen) * r, ty - (tdy / tlen) * r];
}

function arcStartShortened(sx: number, sy: number, mx: number, my: number, tx: number, ty: number, r: number): [number, number] {
  const tdx = mx - sx, tdy = my - sy;
  const tlen = Math.sqrt(tdx * tdx + tdy * tdy) || 1;
...
export function renderMap(
  ...
  // ── Route style for this word
  const routeStyle = getRouteStyle(data.route);

  // ── Defs
  const defs = svg.append("defs");

  // Arrow markers — one per edge type using route color for the primary marker
  const edgeTypes = ["inh", "bor", "der", "lbor", "cog"];
  const edgeColors: Record<string, string> = {
    inh: "#315A78", bor: "#8A2E2E", der: "#556B4D", lbor: "#7B5B2E", cog: "#8B7355",
  };
  edgeTypes.forEach((t) => {
    defs.append("marker")
      .attr("id", "marr-" + t)
      ...
      .attr("fill", "none").attr("stroke", edgeColors[t])
      .attr("stroke-width", 1.5).attr("stroke-linecap", "round");
  });

  // Label halo filter
  const filter = defs.append("filter").attr("id", "lbl-halo")
    .attr("x", "-20%").attr("y", "-20%").attr("width", "140%").attr("height", "140%");
  filter.append("feFlood").attr("flood-color", "#F0E6D2").attr("flood-opacity", "0.85").attr("result", "bg");
  filter.append("feMorphology").attr("in", "SourceGraphic").attr("operator", "dilate").attr("radius", "2").attr("result", "expanded");
  filter.append("feComposite").attr("in", "bg").attr("in2", "expanded").attr("operator", "in").attr("result", "halo");
  ...
  if (_worldTopo) {
    const countries = topojson.feature(_worldTopo as any, (_worldTopo as any).objects.countries);
    gMap.selectAll(".land").data((countries as any).features)
      .enter().append("path").attr("class", "land")
      .attr("d", path as any);
    gMap.append("path").attr("class", "map-border")
      .datum(topojson.mesh(_worldTopo as any, (_worldTopo as any).objects.countries, (a: any, b: any) => a !== b))
      .attr("d", path as any);
  }
  ...
  // ── Build geo nodes (only entries with known coordinates)
  interface GeoNode extends GraphNode { lat: number; lon: number; name: string }
  ...
  const visEdges = data.edges.filter((e) => {
    const t = e.type;
    const ok = (t === "lbor" && (filters.has("lbor") || filters.has("bor"))) || filters.has(t);
    if (!ok) return false;
    const k = e.src + "|" + e.tgt;
  ...
  if (filters.has("cog")) {
    data.cognates.slice(0, 30).forEach((cog) => {
      ...
      if (!gm.has(cog.tgt)) {
        ...
        const gn: GeoNode = { id: cog.tgt, word, lang: lc, lang_name: lang.name, pos: "", etym_text: cog.expansion, lat: lang.lat, lon: lang.lon!, name: lang.name };
        geoNodes.push(gn);
        gm.set(cog.tgt, gn);
      }
      visEdges.push({ src: cog.src, tgt: cog.tgt, type: "cog", expansion: cog.expansion });
    });
  }

  // ── Draw arcs
  // Build arc paths using routeStyle for this word's transmission category
  visEdges.forEach((edge) => {
    ...
    // Shorten endpoints to clear node circles
    const mx = (sx + tx) / 2, my = (sy + ty) / 2;
    const [asx, asy] = arcStartShortened(sx, sy, mx, my, tx, ty, srcR);
    const [atx, aty] = arcEndShortened(sx, sy, mx, my, tx, ty, tgtR);

    const arcColor = isCog ? edgeColors.cog : routeStyle.color;
    const arcDash = isCog ? "3 3" : routeStyle.strokeDash;

    // Build path using RouteStyle geometry scaffold
    const d = buildArcPath(asx, asy, atx, aty, routeStyle);
    ...
    gArcs.append("path")
      ...
      .attr("data-src-lon", src.lon).attr("data-src-lat", src.lat)
      .attr("data-tgt-lon", tgt.lon).attr("data-tgt-lat", tgt.lat)
      .attr("data-src-r", srcR).attr("data-tgt-r", tgtR)
  ...
  geoNodes.forEach((node) => {
    ...
    const r = node.lang === "en" ? NODE_R.en : proto ? NODE_R.proto : NODE_R.default;
    const color = nodeColor(node.lang);

    const ng = gNodes.append("g")
      .attr("class", "m-node-g")
      .attr("data-lon", node.lon).attr("data-lat", node.lat)
    ...
    if (r >= 10) {
      ng.append("text")
        ...
        .text(() => node.word.length > 6 ? node.word.slice(0, 5) + "…" : node.word);
    }

    ng.append("text")
      .attr("class", "m-lbl")
      .attr("text-anchor", "middle").attr("dy", r + 10)
      .attr("font-family", "'IBM Plex Sans',sans-serif")
      .attr("font-size", 8).attr("font-weight", "500")
      .attr("letter-spacing", "0.03em").attr("fill", "var(--ink1)")
      .attr("filter", "url(#lbl-halo)").attr("pointer-events", "none")
  ...
  function repositionNodes(k: number, tx: number, ty: number): void {
    gNodes.selectAll<SVGGElement, unknown>(".m-node-g").each(function () {
      ...
      el.attr("transform", `translate(${px * k + tx},${py * k + ty})`);
    });
  }
}
